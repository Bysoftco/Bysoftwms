...
class ReubicacionesModelo extends BDControlador {
  ...
  function listadoReubicaciones($arreglo) {
    ...
    $query = "SELECT p.nombre AS nombre_ubicacion,ru.codigo,ru.fecha_reubica,cl.numero_documento AS documento,cl.razon_social AS nombre_cliente,
                ref.codigo_ref,ref.nombre AS nombre_referencia,do_asignados.doc_tte,do_asignados.do_asignado AS orden
  ...
  function Reubicar($arreglo) {
    ...
    for($i=1;$i<=$arreglo['nr'];$i++) {
      ...
      if(!empty($arreglo[$reubica])&&(is_numeric($arreglo[$nubica]))) {
        ...
        $query = "UPDATE referencias_ubicacion SET ubicacion = $arreglo[$nubica],
                    inicio = $arreglo[$nubica], fecha_reubica = '$arreglo[fechar]'
