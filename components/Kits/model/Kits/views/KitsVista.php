...
class KitsVista{
    ...
    function retornarDisponiblesKits($idKit, $docCliente){
        ...
        foreach($infoDetalle as $key => $value){
            ...
            $idReferencia = isset($datosReferencia->codigo)?$datosReferencia->codigo:0;
            ...
            if($primera){
                if($enKit>0){
                    $cantDisponibleNal=(isset($disponbibles->cantidad_naci)?$disponbibles->cantidad_naci:0)/$enKit;
                    $cantDisponibleNoNal=(isset($disponbibles->cantidad_nonac)?$disponbibles->cantidad_nonac:0)/$enKit;
                    $cantDisponibleMixto=(isset($disponbibles->cantidad_mixto)?$disponbibles->cantidad_mixto:0)/$enKit;
                }
                $primera=false;
            }
            else{
                if($enKit>0){
                    $Nal=(isset($disponbibles->cantidad_naci)?$disponbibles->cantidad_naci:0)/$enKit;
                    $NoNal=(isset($disponbibles->cantidad_nonac)?$disponbibles->cantidad_nonac:0)/$enKit;
                    $Mixta=(isset($disponbibles->cantidad_mixto)?$disponbibles->cantidad_mixto:0)/$enKit;
    ...
    function agregarKit($arreglo){
        ...
        $this->template->setVariable('titulo_accion', 'Formulatio Kit' );
        $this->template->setVariable('documento_cliente' , isset($datosCliente->numero_documento)?$datosCliente->numero_documento:'');
        $this->template->setVariable('nombre_cliente' , isset($datosCliente->razon_social)?$datosCliente->razon_social:'');
    ...
    function mostrarKitsAlistados($idRegistro){
        ...
        $this->template->setVariable('cantidad', $datosMaestro->cantidad);
        $this->template->setVariable('cantidad_nac', $datosMaestro->cantidad_nac);
        $this->template->setVariable('cantidad_ext', $datosMaestro->cantidad_ext);
        ...
        foreach($detalleAlistamiento as $valueDetalle){
            ...
            $this->template->setVariable('cantidad_nacional', $valueDetalle['cantidad_naci']);
            $this->template->setVariable('peso_nacional', $valueDetalle['peso_naci']);
            $this->template->setVariable('valor_cif', number_format($valueDetalle['cif'],2,'.',','));
            $this->template->setVariable('cantidad_extranjera', $valueDetalle['cantidad_nonac']);
            $this->template->setVariable('peso_extranjera', $valueDetalle['peso_nonac']);
            $this->template->setVariable('valor_fob', number_format($valueDetalle['fob_nonac'],2,'.',','));
            $this->template->parseCurrentBlock("ROW");
        }
