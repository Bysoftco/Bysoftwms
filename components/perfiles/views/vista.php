...
class PerfilesVista {
	...
	function listadoPerfiles($arreglo) {
		$this->template->loadTemplateFile('perfiles/views/tmpl/listadoPerfiles.php');
		...
		if(isset($arreglo['alerta_accion'])) {
			...
			$this->template->setVariable('alturax','50px;');
			$this->template->setVariable('altura','0px;');
		} else {
			$this->template->setVariable('alturax','0px;');
			$this->template->setVariable('altura','42px;');
		}
	...
	function verPerfil($arreglo) {
		$this->template->loadTemplateFile('perfiles/views/tmpl/verPerfil.php');
		$this->template->setVariable('COMODIN','');
	...
	function editarPerfil($arreglo) {
		$this->template->loadTemplateFile('perfiles/views/tmpl/editarPerfil.php');
		$this->template->setVariable('COMODIN','');
