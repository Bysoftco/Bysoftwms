...
class perfiles{
	...
	function editarPerfil($arreglo){
		if(isset($arreglo['id']) && is_numeric($arreglo['id'])){
			...
			$arreglo['alerta_accion'] = 'Perfil Editado con Exito';
	    }
		else{
	        $id = null;
			$arreglo['alerta_accion'] = 'Perfil Creado Con Exito';
	    }
	...
	function eliminarPerfil($arreglo){
		if(is_numeric($arreglo['id'])){
			...
			if($deleted === false){
              set_flash('error', 'La informacion ingresada no es correcta');
	        }
	        else{
              if($deleted === null){
                  set_flash('error', 'Los datos ingresados para la operacion no son vรกlidos');
              }
              else{
                  set_flash('notice', 'El registro se elimino correctamente');
              }
	        }
		}
		else{
          set_flash('error', 'El dato ingresado no es vแlido.');
      	}
		$this->listadoPerfiles($arreglo);
	}
