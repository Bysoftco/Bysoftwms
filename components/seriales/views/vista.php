...
class SerialesVista{
  ...
  function imprimirSerial($arreglo) {
    $this->template->loadTemplateFile( COMPONENTS_PATH . 'seriales/views/tmpl/imprimirSerial.php' );
    $this->template->setVariable('COMODIN', '' );

    //Asigna datos del serial consultado
    $this->template->setVariable('serial', $arreglo['serial']);
    $this->template->setVariable('orden', $arreglo['orden']);
    $this->template->setVariable('doc_tte', $arreglo['doc_tte']);
    $this->template->setVariable('ubicacion', $arreglo['ubicacion']);
