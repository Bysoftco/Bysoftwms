...
class ExistenciasModelo extends BDControlador {
  ...
  function listadoExistencias($arreglo) {
	...
	$arreglo[GroupBy] = "orden,inventario_entrada";
    ...
    if(!empty($arreglo[fechadesdefe])) $arreglo[where] .= " AND do_asignados.fecha >= '$arreglo[fechadesdefe]'";
    if(!empty($arreglo[fechahastafe])) $arreglo[where] .= " AND do_asignados.fecha <= '$arreglo[fechahastafe]'";
    if(!empty($arreglo[doasignadofe])) $arreglo[where] .= " AND do_asignados.do_asignado = '$arreglo[doasignadofe]'";
    
    $query = "SELECT 
	(SELECT nombre FROM posiciones WHERE codigo =MAX(ubicacion) ) AS nombre_ubicacion,
	orden, doc_tte, inventario_entrada, inventario_entrada AS item, arribo, nombre_referencia, cod_referencia, codigo_ref, documento, inv.fecha,
                SUM(cantidad) AS cantidad, SUM(valor) AS valor, SUM(peso) AS peso, modelo, SUM(p_ext) AS p_ext, SUM(p_nal) AS p_nal, nombre_ubicacion1,
                ...
                SUM(cif_retiro) AS cif_retiro, SUM(cif_proceso) AS cif_proceso, SUM(cif_ensamble) AS cif_ensamble, nombre_cliente, inv.codigo, MAX(fmmx) AS fmmn
              FROM(
			  SELECT im.codigo, im.tipo_movimiento, im.flg_control, ie.cantidad AS cant, do_asignados.do_asignado AS orden, do_asignados.doc_tte AS doc_tte,
                ...
                arribos.manifiesto AS manifiesto, arribos.fmm, p.nombre AS nombre_ubicacion1,
              ...
              WHERE im.inventario_entrada = ie.codigo
				...
				LEFT JOIN
				 (
					SELECT max(item) as item,MAX(ubicacion) as ubicacion 
					FROM referencias_ubicacion
					GROUP BY item
				 ) ref_ubicacion ON inv.inventario_entrada=ref_ubicacion.item
			    LEFT JOIN posiciones ON ref_ubicacion.ubicacion=posiciones.codigo
