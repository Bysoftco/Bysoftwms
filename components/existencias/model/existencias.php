...
class ExistenciasModelo extends BDControlador {
  ...
  function listadoExistencias($arreglo) {
    ...
    if(!empty($arreglo['nitfe']))
      $arreglo['where'] .= " AND do_asignados.por_cuenta = ".$arreglo['nitfe'];
    if(!empty($arreglo['fechadesdefe']))
      $arreglo['where'] .= " AND do_asignados.fecha >= '".$arreglo['fechadesdefe']."'";
    if(!empty($arreglo['fechahastafe']))
      $arreglo['where'] .= " AND do_asignados.fecha <= '".$arreglo['fechahastafe']."'";
    if(!empty($arreglo['doasignadofe']))
      $arreglo['where'] .= " AND do_asignados.do_asignado = '".$arreglo['doasignadofe']."'";
    
    $query = "SELECT MIN(ubi_nac) AS ubic, orden, doc_tte, inventario_entrada, inventario_entrada AS item, arribo, nombre_referencia, cod_referencia, codigo_ref, documento, fecha,
                ...
                SUM(p_ret_ext) AS p_ret_ext, SUM(p_ret_nal) AS p_ret_nal, SUM(p_sptr_ext) AS p_sptr_ext, SUM(p_sptr_nal) AS p_sptr_nal, manifiesto, fmm,
                SUM(p_prtt_ext) AS p_prtt_ext, SUM(p_prtt_nal) AS p_prtt_nal, SUM(p_rpk) AS p_rpk, SUM(p_kit_ext) AS p_kit_ext, SUM(p_kit_nal) AS p_kit_nal,
                ...
                SUM(cif_retiro) AS cif_retiro, SUM(cif_proceso) AS cif_proceso, SUM(cif_ensamble) AS cif_ensamble, nombre_cliente, codigo, MAX(fmmx) AS fmmn
              FROM(SELECT po.descripcion AS ubi_nac, ie.codigo, im.tipo_movimiento, im.flg_control, ie.cantidad AS cant, do_asignados.do_asignado AS orden, do_asignados.doc_tte AS doc_tte,
                ...
                arribos.manifiesto AS manifiesto, arribos.fmm, p.nombre AS nombre_ubicacion,
                ...
                END AS v_rpk,
                im.inventario_entrada, im.fmm AS fmmx 
                FROM inventario_movimientos im
              LEFT JOIN posiciones po ON (po.codigo=im.ubicacion AND po.sede='$sede'),inventario_entradas ie,arribos,do_asignados,clientes,referencias ref,posiciones p
