...
class cargueInventariosModelo extends BDControlador {
  ...
  function validaDatos($arreglo) {
    ...
    for($i = 2; $i <= $arrayCount; $i++) {
      ...
      //Valida registro sin información útil
      if(strlen(trim($arribo))!=0) {
        ...
        //Elimina fila con información no útil
        unset($validador[$i]);
      }
    }
    return $validador;
    ...
    for($i = 2; $i <= $arrayCount; $i++) {
      ...
      if(strlen(trim($arribo))!=0) {           
        ...
        if($codigo!="" && $actualizo) {
          ...
          $actualizo = false;
					//Actualizo la ubicación en la tabla referencias_ubicacion
					$query = "UPDATE referencias_ubicacion
                      SET ubicacion = $posicion,
													inicio = $posicion WHERE (item = $codigo)";
        } else {
