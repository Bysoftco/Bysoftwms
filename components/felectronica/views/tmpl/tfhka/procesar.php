    ...
    */
    ...
    for($i = 0;$i < sizeof($fdetfact->cantidad); $i++) {
      ...
      $objFactImp[$i] = new FacturaImpuestos();
    	...
    	//$objFactImp[$i]->valorTOTALImp = $impgen->totalimpuesto;//valor del impuesto a sumar a la base imponible (preciounitario * cantidad) * tasaimpuesto/100 =>Ejemplo (2500 * 2) * (19 / 100)
        $objFactImp[$i]->valorTOTALImp = $impgen->ivaimpuesto[$i];
    	$factDetalle[$i]->impuestosDetalles[0] = $objFactImp[$i];
        $factura->detalleDeFactura[$i] = $factDetalle[$i];
    }

    //IMPUESTOS GENERALES
    for($i = 0;$i < sizeof($fdetfact->cantidad); $i++) {
      $objImpGen[$i] = new FacturaImpuestos();
    	...
    	$objImpGen[$i]->porcentajeTOTALImp = $impgen->tasaimpuesto[$i];
   		//$objImpGen[$i]->valorTOTALImp = $impgen->totalimpuesto;
        $objImpGen[$i]->valorTOTALImp = $impgen->ivaimpuesto[$i];
   
        $factura->impuestosGenerales[$i] = $objImpGen[$i];
    }
    ...
    $factura->totalDescuentos = "0.00";
    //$factura->totalSinImpuestos = $factura->impuestosGenerales[0]->baseImponibleTOTALImp;//$factura->impuestosGenerales["baseImponibleTOTALImp"];
    $factura->totalSinImpuestos = $ffactura->totalbaseimponible;
    $factura->importeTotal = $ffactura->montototal;
