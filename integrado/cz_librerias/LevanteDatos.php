...
class Levante extends MYDB {
    ...
    if(!empty($arregloDatos[modelo_filtro])) {
		...
		$sql = "SELECT orden,
									...
									TRUNCATE( SUM(cantidad_nonac),1) AS cantidad_nonac,   
									 TRUNCATE(SUM(fob_nonac)) AS fob_nonac,
									TRUNCATE( SUM(cif)) AS cif,
									 cod_maestro,
  ...
  function getCuerpoRetiro($arregloDatos) {
    ...
    $sql = "SELECT im.codigo AS id_retiro,inventario_entrada AS id_item,
              ...
              SUM(peso_nonac) AS peso_nonac,SUM(cantidad_nonac) AS cantidad_nonac,
              SUM(cantidad_naci) AS cantidad_naci,SUM(cif) AS cif,SUM(fob_nonac) AS fob_nonac,
              MAX(arribos.orden) AS orden,MAX(arribos.arribo) AS arribo,
              MAX(ie.codigo) AS cod_item,MAX(ref.nombre) AS nombre_referencia,
              MAX(ref.codigo) AS cod_referencia,MAX(ref.codigo_ref) AS ref_cliente,
              MAX(do_asignados.doc_tte) AS doc_tte,MAX(arribos.ubicacion) AS ubicacion,
              MAX(imm.fmm) as fmm
            FROM inventario_entradas ie,do_asignados,referencias ref,embalajes,arribos,
              inventario_movimientos im
  ...
  function findCliente($arregloDatos) {
    $sql = "SELECT numero_documento,razon_social,correo_electronico
            FROM clientes WHERE razon_social LIKE '%$arregloDatos[q]%'";
...
//echo $sql;
  ...
  function getDeclaracion( $arregloDatos) {	   
    $sql = "SELECT MAX(num_declaracion) AS num_declaracion FROM inventario_declaraciones
            WHERE cod_maestro='$arregloDatos[maestro]'";
