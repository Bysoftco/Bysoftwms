...
class Interfase extends BDControlador {
  var $db;
  ...
  function listarInterfases($arregloDatos) {
    $sql = "SELECT DISTINCT facturas_maestro.fecha_interfase,facturas_maestro.interfase,SUM(facturas_detalle.valor) AS valor FROM facturas_maestro,facturas_detalle WHERE facturas_maestro.codigo=facturas_detalle.factura";
  ...
  function cerrarInterfase($arregloDatos) {
    ...
    $sede = $_SESSION['sede'];
    $sede = 11;
    $sql = "UPDATE facturas_maestro SET interfase = '$arregloDatos[interfase_filtro]',fecha_interfase	= '$fecha'
