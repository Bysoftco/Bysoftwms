...
class LevanteLogica {
  ...
  function newPedido($arregloDatos) {
    ...
    switch($arregloDatos['tipo_retiro']) {
      ...
      case 8: //  proceso
        ...
        $arregloDatos['tipo_retiro_filtro'] = 8;// para que muestre extranjero
        break;
    }
    
    $unDatos = new Levante();
    $arregloDatos['id_levante'] = $unDatos->newLevante($arregloDatos);// siempre se crea un nuevo retiro
    $this->controlarTransaccion($arregloDatos);
    $this->pantalla->maestro($arregloDatos);
  }
  ...
  // Función que al enviar el menú Consulta muestra todos los movimientos
  ...
  function addItemRetiro($arregloDatos) {
    ...
    /*
      Inmediatamente después de hacer el retiro, se verifica si el registro queda o no con inventario
    ...
    */
    ...
    $verificaInventario = $verificaInventario->db->fetch();
    ...
    if($arregloDatos['cantidad_retirada_naci']<=0 && $arregloDatos['cantidad_retirada_ext']<=0) {
      //$unRetiroUbicacion->borrarUbicacionesInventario($arregloDatos); Desactivada el 13/Dic/2023 - Miércoles
  ...
  function updateLevanteCabeza(&$arregloDatos) {
    ...
    if($arregloDatos['parcial']) {
      ...
      if($grupossolicitados > $gruposcreados) { // se decidió no crear el parcial
        $arregloDatos['num_grupo'] = is_null($gruposcreados) ? 1 : $gruposcreados; // se actualiza el número real de grupos
        $this->datos->updateConteoParciales($arregloDatos);
      }
    }
    $arregloDatos['mostrar'] = 1;
  ...
  function updateProcesoCabeza($arregloDatos) {
    if($arregloDatos['cierre']) {  // Se procede a la creación de la nueva Orden
      ...
      if(!$unaOrden->addOrden($arregloDatos)) {
        ...
        //Se actualiza la tabla de movimientos con las cantidades de ext y nac como , se obtiene el ID de inventario_entradas
  ...
  function delMercanciaLevante($arregloDatos) {
    ...
    //grupo_borrado se verifica si se borró todo el parcial de un grupo para restar 1
    $unaConsulta = new Levante();
    $unaConsulta->getConteoParciales($arregloDatos);
    $nrow = count($unaConsulta->db->getArray());
    if($nrow == 0) {
  ...
  function maestroConsulta($arregloDatos) {
    ...
    $arregloDatos['titulo'] = empty($this->titulo($arregloDatos))?' CONSULTA':$this->titulo($arregloDatos);
  ...
  function findBodega($arregloDatos) {
    ...
    $filas = 0;
    while($obj=$unaConsulta->db->fetch()) {
      $nombre = '['.trim($obj->codigo).']'.trim($obj->nombre);
      echo "$nombre|$obj->codigo|$obj->nombre|$obj->direccion|$obj->codigo\n";
      $filas++;
    }
    if($filas == 0) {
      echo "No hay Resultados|0\n";
  ...
  function findPos($arregloDatos) {
    $unaConsulta = new Levante();
    $arregloDatos['q'] = strtolower($_GET["q"]);
    $unaConsulta->findPos($arregloDatos);
  ...
  function findPosicion($arregloDatos) {
    ...
    while($obj=$unaConsulta->db->fetch()) {
      $nombre = '['.trim($obj->codigo).'] '.trim($obj->nombre);
      echo "$nombre|$obj->codigo|$obj->arancel|$obj->direccion|$obj->codigo|$obj->codigo|$iva\n";
      $filas++;
    }
    if($filas == 0) {
      echo "No hay Resultados|0\n";
  ...
  function getDatosSubpartida($arregloDatos) {
    ...
    while($obj=$this->datos->db->fetch()) {
      $nombre = '['.trim($obj->codigo).']'.trim($obj->nombre);
      echo "$nombre|$obj->codigo|$obj->arancel|$iva\n";
    }
  }
  
  function getFOB($arregloDatos) {
    $this->datos->getFOB($arregloDatos);
    $dato = $this->datos->db->fetch();
    echo $dato->valor;    
  }
