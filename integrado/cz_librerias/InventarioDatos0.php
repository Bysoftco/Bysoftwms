...
class Inventario extends MYDB {
  ...
  function listaInventario($arregloDatos) {
    $sql = "SELECT codigo AS item,arribo,':' as una_referencia 
  ...
  function getInventario($arregloDatos) {
    $sql = "SELECT inv.cantidad, inv.codigo AS item, inv.arribo, inv.peso, inv.valor, inv.fmm, inv.referencia, inv.modelo,
  ...
  function addInventario($arregloDatos) {
    ...
    $fecha = date('Y/m/d');
    $sql = "INSERT INTO inventario_entradas(orden, arribo, fecha, referencia, posicion, un_empaque, cantidad, peso, valor)
						VALUES('$arregloDatos[do_asignado]','$arregloDatos[id_arribo]','$fecha',1,1,1,$arregloDatos[cantidad],$arregloDatos[peso_bruto],
										$arregloDatos[valor_fob])";
  ...
  function findReferencia($arregloDatos) {
   ...
   $arregloDatos[qmay]=strtoupper($arregloDatos[q]);
    $sql = "SELECT codigo, nombre, serial, codigo_ref FROM referencias
	...
	$sql .= "  UNION SELECT codigo, nombre, serial, codigo_ref FROM referencias WHERE codigo IN(1,2)";
   //echo $sql;
    $this->query($sql);
    if($this->_lastError) {
      echo $sql;
  ...
  function setNuevoPeso($arregloDatos) {  // 07/09/2016  se actualiza peso del item cuando se divide despues de retiros
   ...
   // El ingreso del inventario original se debe ajustar dado que se dividio la mercancia
  	$sql = "UPDATE inventario_movimientos
			 ...
			 SET peso_nonac		=$arregloDatos[peso],
