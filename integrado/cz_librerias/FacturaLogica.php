...
class FacturaLogica {
    ...
    if($this->datos->updateCabeza($arregloDatos)) {
      ...
      foreach($arregloDatos['id_conceptos'] as $key => $value) {	  
        ...
        if($arregloDatos['una_orden'] <>'') {
          ...
          if(intval($arregloDatos['una_orden']) <> 0) {
  ...
  function findConcepto($arregloDatos) {
    ...
    while($obj=$unaConsulta->db->fetch()) {
      ...
      echo "$nombre|$obj->codigo|$obj->iva|$obj->rte_fuente|$obj->rte_ica|$obj->rte_cree\n";
      $row++;
    }
    if($row == 0) {
  ...
  function getDatosUnaOrden($arregloDatos) {
    ...
    $regs = $this->datos->db->fetch();
    $arregloDatos['datos_remesa'] = "Piezas : ".trim(abs($regs->cantidad));
    $arregloDatos['datos_remesa'] .= "  Peso : ".trim(abs($regs->peso) );
    $arregloDatos['datos_remesa'] .= "  Valor : ".round($regs->valor);
  ...
  function getDatosRemesa($arregloDatos) {
    ...
    $arregloDatos['cliente']= $unCliente->numero_documento;  //para saber de donde saco la mercancia
		...
		if($cantidad > 0) {
			...
			$resultado = $this->datos->db->fetch();
			switch($arregloDatos['tarifa']) {
				case 1:  //plena
				  $tarifa = $resultado->plena;
				  break;   
				case 2: //general
				  $tarifa = $resultado->agente;
				  break;
				case 3: //minima
					$tarifa = $resultado->minima;
				  break;
			}
		}
  ...
  function setRetiroInventario(&$arregloDatos) {
    ...
    $arregloDatos['having'] = " HAVING peso_nonac  > 0 OR peso_naci > 0 ";
		...
		// Se crea la cabeza del retiro
		$paraCabezaRetiro = new Levante();
		$arregloDatos['tipo_movimiento'] = 3;// tipo de movimiento retiro
		...
		$arregloDatos['obs'] = " Retiro desde M&ocaute;dulo de Prefactura $arregloDatos[num_prefactura] ";
    ...
    $paraRetiro = new Levante();
    $paraRetiro->getInvParaProceso($arregloDatos);
    $datos = $paraRetiro->db->getArray();		
		//while($obj=$paraRetiro->db->fetch()) {
    foreach($datos as $obj) {
      // Convertimos el Arreglo a Objeto
      $objRegistro = $this->pantalla->converArrObj($obj);
        ...
        $cantidad_retirada = $objRegistro->cantidad_naci+$objRegistro->cantidad_nonac;			
        /*if($cantidad_retirada <= $arregloDatos[cantidad])
  			{
				...
				//var_dump($paraRetiro);
				$arregloDatos['peso_naci_para'] = $objRegistro->peso_naci;
				$arregloDatos['peso_nonaci_para'] = $objRegistro->peso_nonac;
				$arregloDatos['cantidad_naci_para']	= $objRegistro->cantidad_naci;
				$arregloDatos['cantidad_nonaci_para']	= $objRegistro->cantidad_nonac;
				$arregloDatos['cif_ret'] = $objRegistro->cif;
				$arregloDatos['fob_ret'] = $objRegistro->fob_nonac;
				$arregloDatos['id_item'] = $objRegistro->inventario_entrada;
				$arregloDatos['num_levante'] = $arregloDatos['num_prefactura'];
				$arregloDatos['una_orden']= $objRegistro->orden;
				//var_dump($arregloDatos);
				if($arregloDatos['para_cerrar']==1) {
					$this->getDecideRetiro($arregloDatos,$objRegistro);
				}
  ...
  function getDecideRetiro(&$arregloDatos,$paraRetiro) {
      ...
      $unRetiro->addItemRetiro($arregloDatos);		
    } else { // hay más mercancía en extranjero
      $retiro=$arregloDatos['cantidad']-$arregloDatos['acumula'];
